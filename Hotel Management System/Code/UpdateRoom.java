...
public class UpdateRoom extends JFrame 
{
	...
	public UpdateRoom() throws SQLException 
	{
		...
		Image img=new ImageIcon(this.getClass().getResource("/updateroom.jpeg")).getImage();
		lblNewLabel_1.setIcon(new ImageIcon(img));
		lblNewLabel_1.setBounds(380, 35, 504, 343);
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try
				{
				    ...
				    ResultSet rs1 = st.executeQuery("select * from customertable where number = '"+s1+"'");
				    while(rs1.next())
				    {
					txt_Room.setText(rs1.getString("ARN"));    
                    }
				}
				catch(Exception ee)
				{
				...
				try
				{
					Connection con  = new MyConnection().getConnection();
                    Statement st=con.createStatement();
                    ResultSet rs2 = st.executeQuery("select * from roomtable where RoomNo = "+txt_Room.getText());
                    while(rs2.next())
                    {
                    	txt_Ava.setText(rs2.getString("Availability")); 
                        txt_Status.setText(rs2.getString("CleaningStatus"));
                    }
                 }
				catch(Exception ee)
				{
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) throws NumberFormatException {		
				try
				{
					Connection con = new MyConnection().getConnection();
                    Statement st=con.createStatement();
                    String str = "update roomtable set CleaningStatus = '"+txt_Status.getText()+"' where RoomNo = "+txt_Room.getText();
